...
from .deliver import _post_json, DeliveryError
    ...
    for event in batch['events']:
        ...
        def upload(path, name, key, expected=None, *, tail=False):
            ...
            with path.open('rb') as stream:
                if tail: stream.seek(max(0,size-1024*1024))
                raw=stream.read()
            sha=hashlib.sha256(raw).hexdigest()
            status,value=_post_json(url,'/api/attach',{
                'project_id':batch['project_id'],'target_type':'overview','target_id':batch['project_id'],
                'name':name,'data_base64':base64.b64encode(raw).decode(),'sha256':sha,
                'mime_type':'application/zip' if path.suffix=='.zip' else 'text/plain; charset=utf-8',
                'metadata':{'provider':'research-run','capture_key':key+':'+sha,
                            'external_path':str(path),'machine':snapshot['machine'],
                            'original_size':size,'truncated':len(raw)<size}},token,timeout)
            if not 200<=status<300:
                raise DeliveryError(f'Evidence upload failed (HTTP {status}); event remains pending')
            return value['id']

        snapshot['archive_attachment_id']=upload(snapshot['archive_path'],
            '代码 '+snapshot['commit_hash'][:12]+'.zip','code:'+snapshot['commit_hash'],snapshot['archive_sha256'])
        entire=Path(snapshot['archive_path']).parent/'entire-visible.json'
        if snapshot.get('entire') and entire.is_file():
            snapshot['entire_attachment_id']=upload(entire,'Entire 会话与来源.json','entire:'+snapshot['entire']['checkpoint_id'])
        if run.get('state') in {'COMPLETED','FAILED','CANCELLED','TIMEOUT','OUT_OF_MEMORY'}:
            for name in ('stdout','stderr'):
                if run.get(name) and Path(run[name]).is_file():
                    run[name+'_attachment_id']=upload(run[name],run['name']+' '+name+'.log',run['id']+':'+name,tail=True)
