...
from .deliver import project_binding, read_marker, default_data_dir, find_marker, long_path
...
class Workspace:
    def __init__(self, path='.', *, require_enabled=True):
        ...
        if not self.binding and not require_enabled:
            ...
            if marker:
                ...
                self.binding = {**value, 'marker_path':str(marker), 'project_dir':str(marker.parent),
        ...
        self.state = long_path(common.resolve() / 'research-trace' / hashlib.sha256(str(self.root).encode()).hexdigest()[:12])
        self.state.mkdir(parents=True, exist_ok=True)
        self.limit = int(self.config.get('max_code_bytes', 100 * 1024 * 1024))

    def git(self, *args, data=None, env=None):
        result = subprocess.run(['git', '-c', f'safe.directory={self.root.as_posix()}', '-c', 'core.longpaths=true', *args],
                                cwd=self.root, input=data, capture_output=True, env=env, timeout=60)
    ...
    def files(self):
        ...
        for entry in self.git('ls-files','--stage','-z').decode('utf-8').split('\0'):
            if entry:
                metadata,name=entry.split('\t',1); tracked_modes[name]=metadata.split()[0]
        ...
        for name in sorted(set(names)):
            ...
            if total > self.limit:
                raise WorkspaceError('Code exceeds max_code_bytes; put large datasets/weights in .gitignore and register them as data references')
            mode = tracked_modes.get(name,'100644') if os.name=='nt' else ('100755' if path.stat().st_mode & 0o111 else '100644')
    ...
    def _entire_snapshot(self, files, head, session_id):
        ...
        point,archive=repository.matching_pending(files,session_id=session_id,
            since_ns=self.config.get('started_at',0),exclude_ids=self.config.get('entire_start_checkpoints',()),
        ...
        entire={'provider':'entire','checkpoint_id':point.get('condensation_id') or commit,
                'checkpoint_kind':'persistent' if point.get('is_logs_only') else 'pending',
                'commit_hash':commit,'matches_snapshot':True,'sessions':[{'session_id':point.get('session_id')}]}
        result={'schema':'research-trace.code.v1','provider':'entire','commit_hash':commit,'base_commit':head,
                'tree':self.git('rev-parse',commit+'^{tree}').decode().strip(),'git_ref':ref,'files':files,
                'machine':socket.gethostname(),'archive_path':str(archive_path),'archive_sha256':digest(archive_path),
                'captured_at':time.time_ns(),'session_id':session_id,'entire':entire,
                'coverage':'Entire code tree, excluding hook/session metadata; external data and environment are references'}
        atomic_json(folder/'entire-visible.json',{'metadata':point,'transcripts':[],
    ...
    def snapshot(self, *, session_id=None, require_entire=False):
        with FileLock(str(self.state / 'capture.lock'), timeout=30):
            ...
            if (previous.get('files') == files and previous.get('base_commit') == head
                    and Path(previous['archive_path']).is_file() and digest(previous['archive_path']) == previous['archive_sha256']):
                return previous
            ...
            env = dict(os.environ, GIT_INDEX_FILE=git_index, GIT_AUTHOR_NAME='Research Trace',
                       GIT_AUTHOR_EMAIL='trace@localhost', GIT_COMMITTER_NAME='Research Trace', GIT_COMMITTER_EMAIL='trace@localhost')
            try:
                ...
                if tree == self.git('rev-parse', head+'^{tree}').decode().strip():
                    ...
                    commit = self.git('commit-tree', tree, *parents, data=b'Research Trace workspace checkpoint\n', env=env).decode().strip()
                ref = 'refs/research-trace/checkpoints/' + commit
                self.git('update-ref', ref, commit)
                folder = self.state / 'snapshots' / commit
                folder.mkdir(parents=True, exist_ok=True)
                ...
                archive_path = folder / 'code.zip'
                archive_path.write_bytes(archive)
                result = {'schema': 'research-trace.code.v1', 'provider':'git-code-snapshot',
                          'entire_gap':entire_gap, 'commit_hash': commit, 'base_commit': head,
                          'tree': tree, 'git_ref': ref, 'files': files, 'machine': socket.gethostname(),
                          'archive_path': str(archive_path), 'archive_sha256': digest(archive_path),
                          'captured_at': time.time_ns(), 'session_id': session_id,
                          'entire': None, 'coverage': 'repository code; external data and environment are references'}
                ...
                if executable and head != self.config.get('start_commit'):
                    try:
                        ...
                        result['entire'] = {'checkpoint_id': evidence.checkpoint_id, 'checkpoint_ref': evidence.checkpoint_ref,
                                            'commit_hash': head, 'matches_snapshot': head == commit,
                                            'sessions': evidence.metadata['sessions']}
                        atomic_json(folder / 'entire-visible.json', {'metadata': evidence.metadata, 'transcripts': evidence.transcripts})
                    except Exception as exc:
...
def capture_phase(binding, session_id):
    ...
    for delay in (0, .25, .5, 1):
        if delay:
            time.sleep(delay)
        try:
            return workspace.snapshot(session_id=session_id,require_entire=bool(workspace.config.get('entire_executable')))
...
def main(argv=None):
    ...
    try:
        ...
        if args.action=='init':
            ...
            if args.max_code_mb < 1: raise WorkspaceError('--max-code-mb must be positive')
            config={'enabled':True,'start_commit':workspace.head(),'started_at':time.time_ns(),
                    'outbox':str(Path(args.outbox or workspace.state/'delivery').resolve()),'url':args.url,
            ...
            if entire:
                ...
                subprocess.run([executable,'enable','--agent','claude-code','--no-init-repo','--local',
                                '--skip-push-sessions','--telemetry=false','--absolute-git-hook-path'],cwd=workspace.root,check=True,timeout=60)
                ...
                for groups in settings.get('hooks',{}).values():
                    for group in groups:
                        for hook in group.get('hooks',[]):
                            ...
                            match=re.fullmatch(r"sh -c 'if ! command -v entire .*; exec entire hooks claude-code ([a-z-]+)'",command)
                            if match:
                                binary=Path(executable).as_posix()
                                hook['command']='sh -c '+shlex.quote('exec '+shlex.quote(binary)+' hooks claude-code '+match[1])
                atomic_json(settings_path,settings)
                config['entire_executable']=executable
                from .entire_evidence import EntireRepository
                config['entire_start_checkpoints']=[point['id'] for point in EntireRepository(workspace.root,executable).pending()]
            workspace.config=config; workspace.limit=config['max_code_bytes']
            ...
            result=[json.loads(path.read_text(encoding='utf-8')) for path in sorted((workspace.state/'snapshots').glob('*/snapshot.json'))]
        def summary(value):
            output=dict(value)
            if 'files' in output: output['file_count']=len(output.pop('files'))
            return output
        print(json.dumps([summary(item) for item in result] if isinstance(result,list) else summary(result),ensure_ascii=False,indent=2))
        return 0
    except Exception as exc:
