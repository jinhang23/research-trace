 ...
 * lineage / detail / 面包屑 / 轨道亮起全部由它派生。
 *
 * 搜索只 dim 不 hide：隐藏行会让行号不连续，破坏轨道对齐，也破坏图的形状——
 * 而图的形状本身就是信息（"这里分了三条支"）。搜索的目的是定位，不是过滤。
 */
(function () {
  "use strict";
  ...
  function fileURL(s, rel) {
    if (MODE === "static") return "steps/" + s.dirname + "/" + rel;
    return BASE + "/files/" + encodeURIComponent(s.id) + "/" + rel.split("/").map(encodeURIComponent).join("/");
  }
  ...
  function refresh() { return api("/api/forest").then(apply).catch(fail); }
  ...
  function renderRails() {
    ...
    var svg = $("#rails");
    var w = PAD * 2 + Math.max(0, (F.lane_count || 1) - 1) * LW;
    var h = Math.max(F.steps.length * RH, 1);
    svg.setAttribute("width", w); svg.setAttribute("height", h);
    svg.setAttribute("viewBox", "0 0 " + w + " " + h);

    var x = function (l) { return PAD + l * LW; };
    var y = function (r) { return r * RH + RH / 2; };
    var out = [];

    // 跨轨道的边：沿父的轨道向下，在**子节点那一行**才拐进子的轨道。
    // 拐点落在子那一行，多个子从同一父分叉时视觉上是分开的，不会堆成一团。
  ...
  function renderRows() {
    $("#rows").innerHTML = F.steps.map(function (s) {
      return '<div class="row s-' + s.status + '" data-id="' + esc(s.id) + '">'
        ...
        + (s.author ? '<span class="who">' + esc(s.author) + "</span>" : "")
  ...
  function renderSelection() {
    ...
    document.querySelectorAll("#rails [data-id]").forEach(function (el) {
  ...
  function patch(id, body) {
    return api("/api/steps/" + encodeURIComponent(id), {
      method: "PATCH",
  ...
  function upload(s, file) {
    return fetch(BASE + "/api/steps/" + encodeURIComponent(s.id) + "/files/" + encodeURIComponent(file.name), {
      method: "PUT",
  ...
  function submitNew() {
    ...
    api("/api/steps", {
      ...
      return refresh().then(function () { select(step.id); toast("已创建 " + step.id); });
    }).catch(fail);
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (goto) { e.preventDefault(); select(goto.getAttribute("data-goto")); return; }

    var row = e.target.closest("#rows .row");
    ...
    if (rm) {
      ...
      api("/api/steps/" + encodeURIComponent(s0.id) + "/files/" + path.split("/").map(encodeURIComponent).join("/"),
          { method: "DELETE" }).then(refresh).catch(fail);
      return;
    }

    var st = e.target.closest("[data-status]");
    if (st) { patch(selected(), { status: st.getAttribute("data-status") }).catch(fail); return; }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowDown" || e.key === "j") {
      e.preventDefault();
      select(F.steps[Math.min(F.steps.length - 1, i + 1)].id);
      var r = document.querySelector('#rows .row[data-id="' + selected() + '"]');
      if (r) r.scrollIntoView({ block: "nearest" });
    } else if (e.key === "ArrowUp" || e.key === "k") {
      e.preventDefault();
      select(F.steps[Math.max(0, i <= 0 ? 0 : i - 1)].id);
      var r2 = document.querySelector('#rows .row[data-id="' + selected() + '"]');
  ...
  if (MODE === "static") {
    $("#btn-new").hidden = true;
    $("#btn-token").hidden = true;
    $("#live").className = "dot";
    $("#live").title = "静态导出 — 只读";
  }

  var boot = document.getElementById("forest-data");
  var raw = boot ? boot.textContent.trim() : "";
  ...
  if (MODE === "server") {
    ...
    es.onmessage = function (m) {
      ...
      if (v !== seen) { seen = v; if (!editing) refresh(); }
    };
  }
})();
