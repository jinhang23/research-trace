...
def test_real_entire_pending_snapshot_and_live_session_export(tmp_path, monkeypatch):
    ...
    upstream(
        'enable', '--agent', 'claude-code', '--local', '--no-init-repo', '--skip-push-sessions', '--telemetry=false'
    )
    ...
    upstream(
        ...
        'claude-code',
        'user-prompt-submit',
        payload={**base, 'hook_event_name': 'UserPromptSubmit', 'prompt': 'Compare shared code'},
    )
    ...
    with transcript.open('a', encoding='utf-8') as stream:
        stream.write(
            row(
                'assistant',
                [
                    {'type': 'text', 'text': 'Synthetic result: version two ready.'},
    ...
    hook.handle(
        {**stop, 'transcript_path': str(tmp_path / 'does-not-exist')},
        tmp_path / 'outbox',
        ROOT / 'hooks/RECORDER_PROTOCOL.md',
    )
    ...
    hook.handle(
        {**base, 'hook_event_name': 'UserPromptSubmit', 'transcript_path': str(tmp_path / 'missing')},
        tmp_path / 'outbox',
        ROOT / 'hooks/RECORDER_PROTOCOL.md',
    )
    ...
    with transcript.open('a', encoding='utf-8') as stream:
        stream.write('{"type":')
    with repository.session_stream(sid) as stream:
