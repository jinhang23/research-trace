...
def test_authlib_code_exchange_and_authenticated_identity(monkeypatch):
    ...
    monkeypatch.setattr(
        auth, 'OAuth2Client', lambda **kwargs: OAuth2Client(**kwargs, transport=httpx.MockTransport(respond))
    )
...
def test_official_sdk_handshake_tools_and_validation():
    async def check():
        params = StdioServerParameters(
            command=sys.executable, args=['-m', 'research_trace.mcp', '--url', 'http://127.0.0.1:1']
        )
