...
def git(root, *args):
    return (
        subprocess.run(
            ['git', '-c', 'safe.directory=' + root.as_posix(), *args], cwd=root, capture_output=True, check=True
        )
        .stdout.decode()
        .strip()
    )
...
def bound_workspace(root, project_id='test-project'):
    ...
    (root / 'common').mkdir()
    (root / 'common/value.py').write_text('VALUE = 1\n')
    (root / 'model_a').mkdir()
    (root / 'model_a/train.py').write_text('from common.value import VALUE\nprint(VALUE)\n')
    git(root, 'add', '.')
...
def test_normal_agent_sbatch_is_recorded_without_trace_submitting_or_copying_code(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(tmp_path / 'central')) as client:
        ...
        transcript.write_text('{"type":"user","message":"old"}\n')
        ...
        hook.handle(
            {
                **base,
                ...
                'tool_use_id': 'tool-123',
                'tool_input': {'command': command},
                'tool_response': {'stdout': 'Submitted batch job 12345'},
            },
            tmp_path / 'outbox',
            protocol,
        )
        ...
        node = client.post(
            ...
            json={
                'project_id': pid,
                'idempotency_key': 'passive-group',
                'title': 'Agent submitted a job; results pending',
...
def test_old_run_records_and_code_attachments_remain_readable_without_executor(tmp_path, monkeypatch):
    with TestClient(create_app(tmp_path / 'central')) as client:
        ...
        run = {
            ...
            'wandb_url': 'https://wandb.ai/test/project/runs/example',
            'command': ['python', 'model_a/train.py'],
            'source_dir': 'historical-source-directory',
        }
        ...
        node = client.post(
            '/api/record',
            json={'project_id': pid, 'idempotency_key': 'old-node', 'title': 'Old result', 'run_ids': [run['id']]},
        ).json()
        ...
        store.ingest(
            ...
            events=[
                {
                    'event_id': 'old-run-earlier',
                    'event_type': 'research_run',
                    'payload': {'research_run': {**run, 'revision': 1, 'state': 'RUNNING'}},
                }
            ],
        )
...
def test_fresh_setup_preserves_existing_marker_and_does_not_import_history(tmp_path):
    ...
    value.pop('code_capture')
    value['my_setting'] = 'preserve'
