...
def test_heir_is_chosen_by_subtree_height_not_by_subtree_size():
    """防的是：用「子树有多少个节点」冒充「子树有多深」来选主线。

    这两个量只在链状子树上碰巧相等，所以必须用一棵**大小与高度不同序**的树才测得出来
    （上面那个用例里 002 和 003 的大小、高度同序，两种实现都能过，等于没测）。

    这里 002 是死胡同：它自己下面挂了 5 个一次性小试验，子树大小 6、高度只有 1。
    003 才是真正走下去的那条线：4 步链，子树大小 5、高度 4。
    按大小选会挑中 002，主线被甩到 lane 1；按高度选（规格）才挑中 003。
    """
    pairs = [("001", None), ("002", "001"), ("003", "001")]
    pairs += [(f"{i:03d}", "002") for i in (10, 11, 12, 13, 14)]   # 死胡同下的 5 个小试验
    pairs += [("020", "003"), ("021", "020"), ("022", "021"), ("023", "022")]
...
def test_heir_ties_on_height_go_to_the_smallest_id():
    """两支一样深时取 id 序最小的——必须是确定性的，否则每次编译图都会跳。"""
    pairs = [("001", None), ("002", "001"), ("002b", "001"),
             ("003", "002"), ("004", "002b"), ("005", "004")]
    # 002 支：002→003，高度 1；002b 支：002b→004→005，高度 2 —— 先确认不是平局时按高度走
    _, _, _, lane, _ = mk(*pairs)
    assert lane["002b"] == 0 and lane["004"] == 0 and lane["005"] == 0

    # 改成两支等高（各 1 层），此时应当回退到 id 序：002 赢 002b
    _, _, _, lane2, _ = mk(("001", None), ("002", "001"), ("002b", "001"),
                           ("003", "002"), ("004", "002b"))
    assert lane2["002"] == 0 and lane2["003"] == 0
    assert lane2["002b"] == 1
