...
- README 按「怎么用」重构：5 分钟上手、日常怎么用、常用命令表、出问题先看这里（现象 → 原因 → 处理）、部署服务端；
  原理和取舍全部移到 docs/。
...
- `PATCH /api/nodes/{id}` 的字段放在 body 顶层而不是 `patch` 里时，以前按空 patch 处理：200、version+1、
  写一条与上一版逐字相同的修订，Node 一字不变——UF a36 复验时正好这样"通过"了一次。现在：字段放错层
  400 并告诉你放到 `patch` 下；`patch` 缺失或为空 400；storage 层对什么都没变的 patch 直接返回当前值，
  不 bump version、不留修订（网页"打开编辑框直接保存"和机器重放同一 patch 都是这种情况）。
...
- UF 消融三轮：「基线定了之后，第一个消融…」被写成同一 Inbox 里的第二条根，和基线链断开，「这个消融是在
  哪个基线上做的」丢了；同一子线内的第 2、3 条却能正确承接。跨批次的首次连接是弱项，提示词和 parent_id
  的 schema 说明都写明：证据说明工作起于记忆里已有的结果时，那条 Node 就是 parent，新子线仍然挂在它所依据
  的东西下面；只有真正全新的线才是根。a33 起 UF 三轮验证：a32 自动投递 20 秒内归零、a31 后无一次 format。
