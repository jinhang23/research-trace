...
已有 Frappe 队列时，不默认再加入 LangGraph。只有 Recorder 确实需要跨步骤持久化、暂停恢复等编排能力，才用 [LangGraph persistence](https://docs.langchain.com/oss/python/langgraph/persistence) 替换相应实现。框架不能自动继承目前 Claude fork 的上下文与计费特性；重放也可能重新触发模型调用或外部动作。
