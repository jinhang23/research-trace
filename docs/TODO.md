...
状态（2.0.0a35）：方案 3 的独立 Recorder 已接入，hook 只保存、独立 watcher 消费；在本机把
真 hook → 投递 → Recorder → 网页整条链路跑通，安全分三档，包与格式已标准化。做过什么、
修过什么在 [CHANGELOG](../CHANGELOG.md)。这里只放还没做的。

## 待用户拍板

- [ ] **跨章 parent 要不要放开。** 现在 parent 必须同章（`storage._assert_parent_locked`），消融承接别章的基线时
  parent 留空、正文点名（UF 分章验证：正文带了基线参数，人读没问题）。代价是结构图里两条是独立的根，机器无法
  沿链遍历「这个结论依赖哪些基线」。放开是数据模型改动：网页按章画图、Chapter 摘要的含义都要重定义。
