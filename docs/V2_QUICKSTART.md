...
callback 必须与 `TRACE_V2_PUBLIC_URL` 对应；生产环境必须是 HTTPS。然后在服务器的私密环境
配置（例如只对服务账号可读的 systemd `EnvironmentFile`）中设置：

```bash
TRACE_V2_PUBLIC_URL=https://trace.example.org
TRACE_V2_GITHUB_CLIENT_ID=<oauth-app-client-id>
TRACE_V2_GITHUB_CLIENT_SECRET=<oauth-app-client-secret>
TRACE_V2_SESSION_SECRET=<至少32字符的独立随机值>
TRACE_V2_GITHUB_ADMINS=jinhang23
TRACE_V2_GITHUB_ALLOWED_USERS=collaborator-a,collaborator-b
```
...
- `TRACE_V2_GITHUB_ADMINS`：首次登录即为 admin；至少建议保留一个；
- `TRACE_V2_GITHUB_ALLOWED_USERS`：允许登录，首次为 member；
- `TRACE_V2_GITHUB_ALLOWED_ORG=<org>`：允许该组织的 active member，会额外请求 `read:org`；
- `TRACE_V2_GITHUB_ALLOW_ALL=true`：允许任意 GitHub 用户，不建议用于团队私有记录。
...
仅在本机开发时，可以使用 `TRACE_V2_PUBLIC_URL=http://127.0.0.1:8765` 和
`TRACE_V2_INSECURE_COOKIES=true`。这个开关拒绝非 loopback 地址，不能用于跨机器部署。
...
```bash
trace-v2-login --url https://trace.example.org --logout
```

管理员禁用一个 GitHub 用户时，其网页 session 和全部设备凭证会同时失效。服务端仍接受
`TRACE_V2_TOKEN` 作为旧部署迁移兼容项，但新机器不需要配置它。
...
```bash
trace-v2-backup sync-git --data-dir /srv/research-trace/data \
...
本身。运行中的 `trace-v2.sqlite3`、WAL、待批准 device code、网页 session、设备凭证原文、
GitHub access token 和所有 secret 都不会进入备份。

## 6. 验证与从空库恢复

```bash
trace-v2-backup verify --source /srv/research-trace/private-backup/research-trace-backup
