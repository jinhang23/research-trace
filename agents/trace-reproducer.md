...
- **不替作者结掉岔路口。** 要复现的那一步可能是一组**互斥候选**里的一条
  ...
  更不要去把兄弟候选标成 `dead`——「选了哪个」在这套系统里就是**其余候选标 `dead`**，
  那一下等于替作者做了研究决定，而且做完之后连一条 `moved:` 那样的痕迹都没有。
  某条候选真的跑不通，就老实写一条 `repro: failed`（那是事实），然后把
  「这条走不通，要不要把它标 dead 并结掉 011 那个岔路口」写进「要问作者的」。
