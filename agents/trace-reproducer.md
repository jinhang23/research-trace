...
（分了章节就带上 `chapter="消融实验"`，未分章那一组写 `chapter="-"`），
...
一个项目内部可以分成并列的几块——主实验 / 消融实验 / 数据准备（一行 `chapter:`
写在开启那条线的那一步上，整条子树沿 `parent` 继承）。**每一章各有自己的定稿流程**：
`result:` 指的那一步在哪一章，那条流程就属于哪一章。

所以「复现这个项目」在分了章的项目里**不是一个明确的范围**。派你来的提示说的是
哪一章，就只跑那一章那条链：「复现消融那一段」是常见、也是最省机时的一种要求
（主实验的产物往往还在，消融只是对着它多跑几遍）。

- **范围里必须写清是哪一章**，报告里也要写：「复现的是消融实验这一章的 031」，
  而不是「复现了这个项目」。下一个人第一件事就是核对你跑的和他以为的是不是同一条链
- 项目有哪几章、各自的成果是哪一步，用 `mcp__plugin_research-trace_trace__trace_read`
  的 `chapters=true` 拿。**别自己按章节重算一遍**——归属是沿 `parent` 继承出来的
- **借来的上游要认出来**。消融那一章的流程里会有几步属于主实验（它吃着主实验的
  `023`）——它们是**跨章节的边**带进来的，同样要凑齐，但它们是**别人那一章的步骤**：
  发现它们缺记录时写进「要问作者的」，别当成本章的窟窿去补
- 提示里没说是哪一章、而项目分了好几章，就**停下来交还协调者**（你没有问人的工具），
  别默认「整个项目」或者「最新那一章」。猜一条链跑掉的机时和猜一个超参一样要不回来
- **章节不是分叉。**主实验和消融**同时成立**，跑通消融不代表要把主实验标 `dead`
  ——那是候选组那一套（见硬规矩最后几条），和章节没有关系
...
项目分了章节的话，**先写清是哪一章**，并把借自别章的那几步单独标出来
（「`001` `013` `023` 借自主实验」）。
...
- **不改章节归属。** 复现时你会看清「这一步其实属于消融」，但改 `chapter:`
  改的是**整条子树**的归属，等于替作者重排了论文的章节结构（而且改 `parent`
  也会顺带换章）。写进「要问作者的」，和树形挂错了是同一条规矩。
