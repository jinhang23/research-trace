...
| `mcp__plugin_research-trace_trace__trace_pipeline` | 读**定稿流程**：产出成果的那条链、整条链的等级、最弱的一步、必须说出来的几条诊断。**「这个结果可靠吗」问的就是它**，见 1.7 |
...
| **定稿流程** | **L1（最弱：003）** | **6 步里 1 步的产物已不存在** |

最后一行是**回答「别人能不能重做出这个结果」的那一个**，要放在最前面说。
没有 `result:`（推不出定稿流程）就在这一行写「未声明成果 —— 见「要问作者的」」，
不要留空，也不要拿整链等级顶上。
