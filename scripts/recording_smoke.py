...
def main():
    ...
    root=Path(args.directory).resolve(); project=root/'project'
    if project.exists(): raise ValueError('Choose a new disposable project directory')
    project.mkdir(parents=True)
    entire=Path(args.entire).resolve()
    package=Path(__file__).resolve().parents[1]
    os.environ['TRACE_HOOK_NO_SPAWN']='1'
    def command(argv,payload=None):
        return subprocess.run([str(x) for x in argv],cwd=project,capture_output=True,text=True,
            input=json.dumps(payload) if payload else None,timeout=90,check=True).stdout
    def git(*argv): return command(['git','-c','safe.directory='+project.as_posix(),*argv])
    def api(path,value=None):
        request=Request(args.url+path,data=json.dumps(value,ensure_ascii=False).encode() if value else None,
                        headers={'Content-Type':'application/json'})
        with urlopen(request,timeout=30) as response: return json.load(response)
    git('init','-b','main'); git('config','user.name','Passive recording verification')
    ...
    (project/'model_a').mkdir(); (project/'model_a/train.sbatch').write_text('#!/bin/bash\npython -m model_a.train\n')
    git('add','.'); git('-c','commit.gpgsign=false','commit','-m','Fixture baseline')
    pid=api('/api/projects',{'name':'被动记录验收 · alpha.24'})['id']
    write_marker(project,project_id=pid,capture=True)
    command([sys.executable,'-m','research_trace.workspace','--project',project,'init','--entire',entire,
             '--outbox',root/'outbox','--url',args.url])
    ...
    transcript=root/'synthetic-session.jsonl'; transcript.write_text('')
    sid=str(uuid.uuid4())
    ...
    transcript.write_text(json.dumps({'type':'user','sessionId':sid,'message':{'role':'user',
        'content':'Synthetic fixture: record this experiment without managing its directory or submitting it.'}})+'\n')
    command([entire,'hooks','claude-code','user-prompt-submit'],{**base,'hook_event_name':'UserPromptSubmit','prompt':'Record the experiment'})
    (project/'model_a/train.py').write_text('from common import VALUE\nprint(VALUE)\n')
    workspace=Workspace(project)
    before=workspace.files()
    tool={**base,'tool_name':'Bash','tool_use_id':'fixture-submit','tool_input':{'command':'sbatch model_a/train.sbatch'}}
    ...
    hook.handle({**tool,'hook_event_name':'PostToolUse','tool_response':{'stdout':'Submitted batch job 12345'}},root/'outbox',protocol)
    with transcript.open('a') as stream:
        stream.write(json.dumps({'type':'assistant','sessionId':sid,'message':{'role':'assistant',
    ...
    node=api('/api/record',{'project_id':pid,'idempotency_key':'passive-fixture','title':'原始 sbatch 命令和代码版本已保存，实验结果尚未确认',
    ...
    report={'project_id':pid,'node_id':node['id'],'unchanged_directory':True,'created_execution_directory':False,
            'submitted_job':False,'code_provider':'entire','source_count':len(events),'url':args.url}
