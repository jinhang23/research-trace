...
from research_trace.storage import Store, Conflict
...
def main():
    ...
    env = dict(os.environ, PATH=str(exe.parent) + os.pathsep + os.environ.get('PATH', ''),
               GIT_CONFIG_GLOBAL=str(global_config), GIT_CONFIG_NOSYSTEM='1',
               GIT_TERMINAL_PROMPT='0', PYTHONDONTWRITEBYTECODE='1', PYTHONUTF8='1')
    commands = []
    ...
    def run(command, *, cwd=repo, payload=None):
        ...
        commands.append({'args': command, 'returncode': result.returncode,
    ...
    run([exe, 'enable', '--agent', 'claude-code', '--no-init-repo', '--local',
    ...
    def session(prompt, files, answer, *, make_commit):
        ...
        payload = {'session_id': sid, 'transcript_path': str(transcript), 'cwd': str(repo), 'permission_mode': 'default'}
        def hook(name, **extra):
            return run([exe, 'hooks', 'claude-code', name], payload={**payload, **extra})
        def append(kind, content):
            value = {'type': kind, 'uuid': str(uuid.uuid4()), 'sessionId': sid,
                     'timestamp': datetime.now(timezone.utc).isoformat(), 'cwd': str(repo),
        ...
        for name, text in files.items():
            ...
            append('assistant', [{'type': 'tool_use', 'id': tool_id, 'name': 'Write',
    ...
    commit_a, _, _ = session('模拟探索：先验证模型 A，公共代码 SCALE=1。', {
        'model_a/train.py': TRAIN,
        'model_a/config.json': json.dumps({'bias': 0, 'learning_rate': 0.01}),
    }, '模型 A 准备完成。这只是版本记录测试，尚无真实蛋白质–配体实验结果。', make_commit='Synthetic model A')
    ...
    commit_b, _, _ = session('模拟探索：准备模型 B，同时修改外层公共代码。', {
        'common/features.py': 'SCALE = 2\n',
        'model_b/train.py': TRAIN,
        'model_b/config.json': json.dumps({'bias': 1, 'learning_rate': 0.02}),
    }, '模型 B 使用 SCALE=2；后续应比较各版本的真实结果。', make_commit='Synthetic model B with updated shared code')
    ...
    for label, snapshot in [('a', snapshot_a), ('b', snapshot_b)]:
        tasks.append(subprocess.Popen([sys.executable, '-B', f'model_{label}/train.py',
                                       '--config', f'model_{label}/config.json'],
                                      cwd=snapshot, env=env, text=True, encoding='utf-8',
    ...
    _, idea_sid, idea_transcript = session(
        ...
        '保留这个待探索方向；本轮没有实施、没有产生新实验。', make_commit=None)
    ...
    run(['git', 'bundle', 'create', bundle, 'refs/heads/codex/entire-smoke',
         evidence_a.checkpoint_ref, evidence_b.checkpoint_ref])
    run(['git', 'bundle', 'verify', bundle])
    restored = root / 'restored'
    restored.mkdir()
    run(['git', 'init', '--initial-branch=codex/restored'], cwd=restored)
    refspecs = [f'{ref}:{ref}' for ref in ('refs/heads/codex/entire-smoke', evidence_a.checkpoint_ref, evidence_b.checkpoint_ref)]
    ...
    replayed_output = json.loads(run([sys.executable, '-B', 'model_a/train.py',
                                     '--config', 'model_a/config.json'], cwd=restored_source))
    assert replayed_output == outputs[0]

    store = Store(root / 'trace-data')
    project = store.create_project('代码与探索追溯试验（模拟数据）', overview=(
    ...
    def ingest(sid, text, key):
        ...
        batch = dict(batch_id=key, project_id=pid, session={'id': sid, 'source': 'entire-synthetic-smoke',
                     'cwd': str(repo), 'metadata': {'synthetic': True}}, agents=[], events=events,
    ...
    for label, evidence in [('A', evidence_a), ('B', evidence_b)]:
        for index, transcript in enumerate(evidence.transcripts):
            source_ids.extend(ingest(transcript['session_id'], transcript['content'], f'entire:{evidence.checkpoint_id}:{index}'))
        with zipfile.ZipFile(io.BytesIO(evidence.code_archive)) as archive:
            text = archive.read('common/features.py').decode()
        code_evidence.append({'file_path': 'common/features.py', 'commit_hash': evidence.commit_hash,
                              'snippet': text, 'content_sha256': hashlib.sha256(text.encode()).hexdigest(),
                              'attribution': 'exact', 'annotation': f'模型 {label} 实际运行的公共代码；Entire {evidence.checkpoint_id}'})
    node = store.record_node(pid, chapter_id=chapter['id'], idempotency_key='comparison',
        title='公共代码变化后，两个模型仍能运行各自版本',
        body=('本轮验证代码留存机制。模型 A 保存后，外层公共代码由 SCALE=1 改为 2，再保存模型 B。'
        ...
        labels=['模拟验证', '代码版本'], source_event_ids=source_ids, code_evidence=code_evidence)
    for label, evidence, output in zip(('A', 'B'), (evidence_a, evidence_b), outputs):
        store.attach(pid, target_type='node', target_id=node['id'], name=f'模型 {label} 完整代码.zip',
                     data_base64=base64.b64encode(evidence.code_archive).decode(), mime_type='application/zip',
                     direction='input',
                     metadata={'provider': 'entire', 'checkpoint_id': evidence.checkpoint_id, 'commit_hash': evidence.commit_hash})
        manifest = {'schema': 'research-trace.entire-smoke.v1', 'synthetic': True,
                    'execution': 'local-subprocess', 'slurm_job_id': None, 'wandb_url': None,
                    'entrypoint': f'model_{label.lower()}/train.py', 'commit_hash': evidence.commit_hash,
                    'command': ['python', '-B', f'model_{label.lower()}/train.py',
                                '--config', f'model_{label.lower()}/config.json'],
                    'working_directory': 'extracted code archive root',
                    'python_version': sys.version, 'external_dependencies': [],
                    'checkpoint': evidence.metadata, 'output': output,
                    'source_archive_sha256': hashlib.sha256(evidence.code_archive).hexdigest()}
        store.attach(pid, target_type='node', target_id=node['id'], name=f'模型 {label} 运行与来源.json',
                     data_base64=base64.b64encode(json.dumps(manifest, ensure_ascii=False, indent=2).encode()).decode(),
                     mime_type='application/json', direction='output',
    ...
    idea = store.record_node(pid, chapter_id=chapter['id'], idempotency_key='untried-idea',
        title='待探索：按配体骨架划分验证集',
        body='想法：检查随机划分是否掩盖了相似配体之间的数据泄漏。尚未实施，也没有实验结论。后续可先核对划分方式，再决定是否值得投入。\n\n这是预置的合成会话，用来验证没有代码提交也能保留想法。',
        labels=['未实施', '模拟记录'], source_event_ids=idea_sources)
    human_body = node['body'] + '\n\n人工补充：这次只证明本地版本隔离成立；集群作业仍待验收。'
    store.update_node(node['id'], {'body': human_body}, expect_version=1, actor_type='human', actor_id='synthetic-reviewer')
    ...
    report = {'schema': 'research-trace.entire-smoke-report.v1', 'entire_version': version,
              'synthetic_hook_input': True, 'real_live_claude_session': False, 'summary_generation': 'prewritten-fixture',
              'project_id': pid, 'node_id': node['id'], 'idea_node_id': idea['id'], 'baseline': baseline,
              'commits': [commit_a, commit_b], 'checkpoints': [evidence_a.checkpoint_id, evidence_b.checkpoint_id],
              'local_outputs': outputs, 'checks': {'real_entire_checkpoints': True, 'shared_code_pinned': True,
              'concurrent_local_runs': True, 'code_and_transcript_restored_from_git_bundle': True,
              'restored_code_rerun_matches': True, 'records_revisions_attachments_restored': True,
              'uncommitted_idea_recorded': True, 'raw_replay_deduplicated': True, 'human_revision_protected': stale_rejected},
              'not_verified': ['live Claude automatic capture/Recorder quality', 'UF/SLURM execution', 'real W&B link',
                               'Entire first-write hidden-content filtering', 'offline delivery', 'GitButler coexistence']}
    store.close()
