...
async def main():
    ...
    parser.add_argument(
        "--keyword-only", action="store_true", help="Skip local embedding download; does not verify vector retrieval"
    )
    ...
    memory_env = {
        "BASIC_MEMORY_CONFIG_DIR": str(root / "bm-config"),
        "BASIC_MEMORY_SEMANTIC_SEARCH_ENABLED": str(not args.keyword_only).lower(),
        "BASIC_MEMORY_SEMANTIC_EMBEDDING_THREADS": "2",
        "PYTHONUTF8": "1",
    }
    ...
    (root / "bm-config" / "config.json").write_text(
        json.dumps(
            {
                "projects": {"trace-demo": {"path": str(root / "knowledge"), "mode": "local"}},
                "default_project": "trace-demo",
            }
        ),
        encoding="utf-8",
    )
    ...
    experiment_id = mlflow.create_experiment(
        "Research Trace synthetic verification", artifact_location=(root / "artifacts").as_uri()
    )
    ...
    project = store.create_project(
        "框架集成演示（模拟数据）", overview="验证证据导入、知识检索和人工修订，不代表真实实验结果。"
    )
    ...
    node = store.record_node(
        ...
        title="Regularization experiment",
        body="Dropout reduces overfitting in this synthetic example. Validation AUC is 0.87; replication is still needed.",
    )
    config = {
        "schema": "research-trace.integrations.v1",
        "sync_interval_seconds": 60,
        "basic_memory": {
            "command": str(Path(args.basic_memory).resolve()),
            "args": ["mcp"],
            "env": memory_env,
            "timeout_seconds": 180,
        },
    ...
    assert any(h["id"] == node["id"] and h.get("retrieval_source") == "basic_memory" for h in response["hits"]), (
        response
    )
    ...
    while True:
        ...
        semantic_hit = any(
            h["id"] == node["id"] and h.get("retrieval_source") == "basic_memory" for h in semantic["hits"]
        )
    ...
    revised = store.update_node(
        node["id"],
        {"body": "人工修订：模拟指标只验证集成；还不能推断方法有效。"},
        expect_version=1,
        actor_type="human",
    ...
    report = {
        ...
        "run_id": run_id,
        "trace_id": trace_id,
        "mlflow_run_import": True,
        "mlflow_trace_import": True,
        "hidden_content_removed": True,
        "duplicate_attachment_prevented": True,
        "basic_memory_mcp_sync_and_search": True,
        "vector_retrieval_verified": semantic_hit and not args.keyword_only,
        "current_human_revision_returned": True,
