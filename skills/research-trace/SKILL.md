...
完整的格式标准在插件根目录的 **`FORMAT.md`**（`${CLAUDE_PLUGIN_ROOT}/FORMAT.md`）。
拿不准怎么写就去读它。核心三条：

1. **可视化必须从已经可读的文本里长出来。** 指标一律用 markdown 表格——人看到
   对齐的列和数值底纹条，你读到的还是同样的数字，信息完全对等。别把数字画成图。
2. **图必须带图注，而且图注写的是结论**，不是"这是一张 loss 曲线"。你看不见图，
   图注是它对你唯一的信息来源。
3. **「为什么」写假设，不写动作。** "试试 DistilBERT"等于没写；
   "基线丢掉了词序，假设上下文表征能补回来"才是。
...
- 不要试图改 `id` 或 `parent`，也不要要求删除步骤——只追加是这套系统的地基
